...
extension UIView {
    ...
    func setImageBackground(named: String) {
        UIGraphicsBeginImageContext(frame.size)
        UIImage(named: named)?.drawInRect(bounds)
    ...
    func addBorder(edges edges: UIRectEdge, colour: UIColor = UIColor.whiteColor(), thickness: CGFloat = 1) -> [UIView] {
        ...
        if edges.contains(.Top) || edges.contains(.All) {
            let top = border()
            addSubview(top)
            addConstraints(NSLayoutConstraint.constraintsWithVisualFormat("V:|-(0)-[top(==thickness)]", options: [],
    ...
    func setRotation(deg: CGFloat) {
        let rad:CGFloat = CGFloat(M_PI) * deg / 180.0
        let rot:CGAffineTransform = CGAffineTransformMakeRotation(rad)
        self.transform = rot
    }
}
